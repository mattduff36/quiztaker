...
const SESS_DIR = path.join(ROOT, 'data', 'sessions');
...
function buildSessionReport(now = new Date()) {
  ...
  const learnIndex = path.join(ROOT, 'data', 'learn', 'index.jsonl');
  ...
  try {
    const quizIndex = JSON.parse(fs.readFileSync(path.join(ROOT, 'data', 'runs', 'index.json'), 'utf8'));
...
function commitSession(report, now = new Date()) {
  ...
  written.push(path.relative(ROOT, recapFile).replace(/\\/g, '/'));
